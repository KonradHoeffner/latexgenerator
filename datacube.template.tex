...
% The 3D
\tikzset{my cs/.style={local cs={m.north west}{m.north east}{1}{0.5*$DEPTH$}}}
\begin{scope}[my cs]
...
\foreach \c in {1,...,$ROWS*MEASURES-1$}
  \draw[if={isodd(\c)}{densely dashed}{}]
    (m-\c-1.south west) -- (m-\c-2.south east) -- ++(up:1);
...
\foreach \st[count=\c from 0, evaluate={\d=int(2*\c+1)}] in {$ZVALUES$}
